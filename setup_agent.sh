...
sudo -u "$PM2_RUN_USER" bash -c "export PATH=$PATH:/usr/bin:/usr/local/bin; pm2 startup systemd -u \"$PM2_RUN_USER\" --hp \"$HOME_DIR_PM2_USER\"" || { echo "Échec de pm2 startup pour l'utilisateur $PM2_RUN_USER. Arrêt du script."; exit 1; }
