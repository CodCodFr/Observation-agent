    ...
    systemctl stop vps-tunnel.service > /dev/null 2>&1 || true
    systemctl disable vps-tunnel.service > /dev/null 2>&1 || true
    rm -f /etc/systemd/system/vps-tunnel.service > /dev/null 2>&1 || true

    # Créer un fichier de service Systemd pour le tunnel SSH
    SERVICE_FILE="/etc/systemd/system/vps-tunnel.service"
    echo "Création du fichier de service Systemd à l'emplacement $SERVICE_FILE"

    # J'ai ajouté l'option -p pour spécifier le port SSH non standard.
    # L'option -o BatchMode=yes est ajoutée pour éviter les prompts de mot de passe en cas d'échec d'authentification par clé.
    ...
    # Utiliser tee avec un here-document pour écrire le fichier de service avec les droits root
    cat << EOF | sudo tee "$SERVICE_FILE" > /dev/null
    [Unit]
    Description=Reverse SSH Tunnel to Backend Server
    After=network.target

    [Service]
    # User sous lequel le service va s'exécuter
    User=$PM2_RUN_USER
    # Home directory de l'utilisateur
    WorkingDirectory=$HOME_DIR_PM2_USER
    # La commande qui lance le tunnel. 'which ssh' pour s'assurer du bon chemin.
    ExecStart=/usr/bin/ssh $SSH_COMMAND_ARGS
    # Redémarre en cas de crash
    Restart=always
    # Attend 10 secondes avant de redémarrer pour éviter une boucle rapide
    RestartSec=10
    ...
    if [ ! -f "$SERVICE_FILE" ]; then
        echo "Échec de la création du fichier de service Systemd. Arrêt du script."
        exit 1
    fi
    echo "Fichier de service Systemd créé avec succès."

    # Recharger le daemon systemd, activer et démarrer le service
    systemctl daemon-reload || { echo "Échec de la commande 'systemctl daemon-reload'. Arrêt du script."; exit 1; }
    systemctl enable vps-tunnel.service || { echo "Échec de la commande 'systemctl enable'. Arrêt du script."; exit 1; }
    systemctl start vps-tunnel.service || { echo "Échec de la commande 'systemctl start'. Arrêt du script."; exit 1; }

    echo "Tunnel SSH inversé lancé avec Systemd et configuré pour démarrer au boot."
